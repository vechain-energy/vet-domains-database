...
export async function setupDatabase(database: string) {
    ...
    db.prepare('CREATE TABLE IF NOT EXISTS records (node TEXT PRIMARY KEY, resolverAddress TEXT, type TEXT, key TEXT, value TEXT, blockHeight INTEGER)').run();
    db.prepare(`CREATE VIEW IF NOT EXISTS primaryNames AS 
        SELECT n.node, n.resolverAddress, n.address, MAX(nr.name) as name, MAX(n.blockHeight) as blockHeight 
            FROM nodes n 
                LEFT JOIN nodes nr 
